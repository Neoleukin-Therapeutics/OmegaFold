...
COPY . /build_dir
WORKDIR /build_dir
ADD requirements.txt /tmp/requiremtns.txt
RUN pip install -r /tmp/requiremtns.txt
RUN rm -rf dist
RUN python3 setup.py sdist bdist_wheel
RUN echo $(basename $(ls dist/OmegaFold*.whl)) > wheel_version
RUN cp dist/OmegaFold*.whl ./omegafold-latest.whl

FROM torch-conda-env as exec
COPY --from=builder /build_dir/omegafold-latest.whl /var/omegafold-latest.whl
COPY --from=builder /build_dir/wheel_version /var/wheel_version
RUN mv /var/omegafold-latest.whl /var/$(cat /var/wheel_version)
